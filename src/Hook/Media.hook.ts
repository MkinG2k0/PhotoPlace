...
export interface useArrMediaProps {
	...
	useEffect(() => {
		arrWidth.map((width) => {
			return window
				...
				.addEventListener('change', (e) => handler(width, e.matches))
		})
		...
		return () => {
			arrWidth.map((width) => {
				return window
					.matchMedia(`(min-width: ${width}px)`)
					.removeEventListener('change', (e) => handler(width, e.matches))
			})
		}
	}, [])
}
